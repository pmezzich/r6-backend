...
func NewMatchReader(in *os.File) (m *MatchReader, // removed: err error) {
	paths, // removed: err := ListReplayFiles(in)
	if // removed: err != nil {
		return
	}
	m = &MatchReader{
...
func (m *MatchReader) Read() error {
	for i := range m.paths {
		if // removed: err := m.read(i); // removed: err != nil {
			return // removed: err
		}
	}
	return nil
...
func (m *MatchReader) FirstRound() (r *Reader, // removed: err error) {
	return m.RoundAt(0)
}

func (m *MatchReader) LastRound() (r *Reader, // removed: err error) {
	return m.RoundAt(m.NumRounds() - 1)
}

func (m *MatchReader) RoundAt(i int) (r *Reader, // removed: err error) {
	if m.rounds[i] == nil {
		if // removed: err := m.read(i); // removed: err != nil {
...
func (m *MatchReader) WriteExcel(out io.Writer) error {
	...
	first := f.NewSheet("Match")
	if f.DeleteSheet("Sheet1"); // removed: err != nil {
		return // removed: err
	}
	...
	for i, r := range m.rounds {
		...
		_, // removed: err := f.NewSheet(sheet)
		c.Sheet(sheet)
...
func ListReplayFiles(root *os.File) ([]string, error) {
	files, // removed: err := root.ReadDir(0)
	if // removed: err != nil {
		return nil, // removed: err
	}
	paths := make([]string, 0)
